...
public partial class f_menuN : fmau
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (!b_log)
            {
                ...
                string b_s = this.ResolveUrl("~/menuN" + khac.Fs_runMode() + ".js");
                ScriptManager.RegisterClientScriptInclude(this.Page, this.GetType(), Guid.NewGuid().ToString(), b_s);
                b_s = "menu_KD('" + Fs_tm("K") + "');";
                ScriptManager.RegisterStartupScript(this, this.GetType(), Guid.NewGuid().ToString(), b_s, true);
                tm.Value = Fs_tm("D");
                string b_file = tm.Value + "/ctmenu";
                b_log = khac.Fb_KTRA_QU(this.Title, b_file, "GXDPIMAE");
            }
            else b_log = false;
        }
        catch { b_log = false; }
        ...
        else
        {
            ...
            this.Lb_dvi.Text = b_se.ma_goc + " - " + b_se.ten_dvi;
