...
public partial class khud_ttt : System.Web.UI.UserControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            string b_s = this.ResolveUrl("~/App_ctr/khud/khud_ttt"+khac.Fs_runMode()+".js");
            ScriptManager.RegisterClientScriptInclude(this.Page, this.GetType(), Guid.NewGuid().ToString(), b_s);
            b_s = "khud_ttt_KD('" + this.ps + "','" + this.nv + "','" + khud_ttt_gchu.ClientID
                + "','" + khud_ttt_GR_dk.ClientID + "','" + this.kdvi + "');";
            ...
            if (this.dong != 0) khud_ttt_GR_dk.hangKt = this.dong;
            if (this.ctr_truoc!="") khud_ttt_GR_dk.ctrT = this.ctr_truoc;
            if (this.ctr_sau!="") khud_ttt_GR_dk.ctrS = this.ctr_sau;
        }
    }
}
