var b_cctcId, b_cctcTao = 'C', b_cctcVtro;
function vb_cctc_KD(tao, Id) {
    b_cctcTao = tao; b_cctcId = Id;
}
// Lay ve
function vb_cctc_P_SL(b_id, b_loai, b_cap, b_dvi, b_ma, b_vtro) {
    try {
        if (b_id == '') b_id = b_cctcId;
        if (C_NVL(b_vtro) != '') b_cctcVtro = b_vtro;
        sed_vb_khac.Fs_CCTC(form_Fs_nsd(), b_id, b_loai, b_cap, b_dvi, b_ma, b_cctcVtro, vb_cctc_P_SL_KQ, P_LOI_CSDL, P_LOI_TGIAN);
        return false;
    }
    catch (ex) { form_P_LOI(ex.message); }
}
function vb_cctc_P_SL_KQ(b_kq) {
    if (Fb_LOI_KTRA(b_kq)) form_P_LOI(b_kq); else vb_cctc_P_TAO(b_kq);
}
// Tao
function vb_cctc_P_TAO(b_kq) {
    try {
        ...
        if (a_kq[2] != '') {
            ...
            for (var i = 0; i < a_kq.length; i++) {
                ...
                b_log = (b_cctcTao == 'C' && (a_s[3] == 'C' || 'DYK'.indexOf(b_cctcVtro) < 0));
                if (b_log) b_s = "vb_cctc_P_CHON(event,'" + b_sp.id + "')";
                else if (a_s[3] != 'C') b_s = "vb_cctc_P_CLICK(event,'" + b_id + "')";
                else b_s = '';
                ...
                if (a_s[3] != 'C') {
                    ...
                    if (b_log) {
                        b_s = "vb_cctc_P_CLICK(event,'" + b_id + "')";
        ...
        if (b_ctrId == b_cctcId) {
            ...
            vb_cctc_HIEN(b_s, b_ctrId);
        }
    }
    catch (ex) { form_P_LOI(ex.message); }
}
// Click
function vb_cctc_HIEN(b_IdM, b_idC) {
...
function vb_cctc_P_CHON(b_event, b_id) {
