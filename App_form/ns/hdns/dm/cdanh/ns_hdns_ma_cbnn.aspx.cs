...
public partial class f_ns_hdns_ma_cbnn : fmau
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            string b_s = this.ResolveUrl("~/App_form/ns/hdns/dm/cdanh/ns_hdns_ma_cbnn" + khac.Fs_runMode() + ".js?v=" + ht_dungchung.b_ver);
            ScriptManager.RegisterClientScriptInclude(this.Page, this.GetType(), Guid.NewGuid().ToString(), b_s);
            ScriptManager.RegisterClientScriptInclude(this.Page, this.GetType(), Guid.NewGuid().ToString(), this.ResolveUrl("~/App_form/Bugger.js"));
            ScriptManager.RegisterStartupScript(this, this.GetType(), Guid.NewGuid().ToString(), "ns_hdns_ma_cbnn_P_KD();", true);
    ...
    protected void FileMau_Click(object sender, EventArgs e)
    {
        ...
        b_dt.TableName = "DATA";
        DataTable b_dtNN = se.Fdt_TG_TRA(this.Title, "DT_NNN");
        b_dtNN.TableName = "DATA1";
        DataSet b_ds = new DataSet();
        b_ds.Tables.Add(b_dt);
        b_ds.Tables.Add(b_dtNN);
        Excel_dungchung.ExportTemplate("Templates/importhdns/ns_hdns_ma_cbnn_mau.xls", b_ds, null, "ns_hdns_ma_cbnn_mau");
    }
    protected void XuatExcel_Click(object sender, EventArgs e)
    {
        try
        {
            DataTable b_dt = ns_hdns.Fdt_NS_HDNS_MA_CBNN_EXCEL();
            bang.P_THAY_GTRI(ref b_dt, "tt", "N", "Ngừng áp dụng");
            bang.P_THAY_GTRI(ref b_dt, "tt", "A", "Áp dụng");
            b_dt.TableName = "DATA";
            Excel_dungchung.ExportTemplate("Templates/ExportMau/ns/hdns/ns_hdns_ma_cbnn.xlsx", b_dt, null, "Danh_muc_chuyen_mon");
