...
public partial class f_ns_dt_trienkhai : fmau
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            string b_s = this.ResolveUrl("~/App_form/ns/dt/ngv/ns_dt_trienkhai" + khac.Fs_runMode() + ".js?v=" + ht_dungchung.b_ver);
            ScriptManager.RegisterClientScriptInclude(this.Page, this.GetType(), Guid.NewGuid().ToString(), b_s);
            se.se_nsd b_se = new se.se_nsd(); string b_nsd = b_se.nsd; string b_so_the = P_SOTHE(b_nsd);
            ScriptManager.RegisterStartupScript(this, this.GetType(), Guid.NewGuid().ToString(), "ns_dt_trienkhai_P_KD('" + b_so_the + "');", true);
            P_NHAN_DROP();
        }
    }
    ...
    protected void btn_excel_mau_Click(object sender, EventArgs e)
    {
        ...
        Excel_dungchung.ExportTemplate("Templates/importmau/ns_dt_trienkhai_ktao.xls", b_ds, null, "ns_dt_trienkhai_ktao");
    }
    protected void nhap_Click(object sender, EventArgs e)
    {
        try
        {
            DataTable b_dt = null;// tl_cc.P_ns_dt_trienkhai_EXPORT();
            ...
            b_dt.TableName = "DATA";
            Excel_dungchung.ExportTemplate("Templates/ExportMau/ns_dt_trienkhai_export.xlsx", b_dt, null, "dangky_dimuon_vesom");
