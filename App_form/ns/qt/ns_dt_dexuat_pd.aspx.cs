...
public partial class f_ns_dt_dexuat_pd : fmau
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!IsPostBack)
            {
                ...
                string b_s = this.ResolveUrl("~/App_form/ns/qt/ns_dt_dexuat_pd" + khac.Fs_runMode() + ".js?v=" + ht_dungchung.b_ver);
                ScriptManager.RegisterClientScriptInclude(this.Page, this.GetType(), Guid.NewGuid().ToString(), b_s);
                ScriptManager.RegisterStartupScript(this, this.GetType(), Guid.NewGuid().ToString(), "ns_dt_dexuat_pd_P_KD('');", true);
                P_NHAN_DROP();
            }
        }
