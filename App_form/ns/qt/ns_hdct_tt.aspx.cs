...
public partial class f_ns_hdct_tt : fmau
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!IsPostBack)
            {
                ...
                string b_s = this.ResolveUrl("~/App_form/ns/qt/ns_hdct_tt" + khac.Fs_runMode() + ".js?v=" + ht_dungchung.b_ver);
                se.se_nsd b_se = new se.se_nsd();
                string b_ma_dvi = b_se.ma_dvi;
                string b_nsd = b_se.nsd;
                string b_so_the = P_SOTHE(b_nsd);
                ScriptManager.RegisterClientScriptInclude(this.Page, this.GetType(), Guid.NewGuid().ToString(), b_s);
                ScriptManager.RegisterStartupScript(this, this.GetType(), Guid.NewGuid().ToString(), "ns_hdct_tt_P_KD('" + b_so_the + "');", true);
            }
        }
