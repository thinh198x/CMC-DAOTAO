...
public partial class f_ns_td_ungvien_nv_pd : fmau
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!IsPostBack)
            {
                ...
                string b_s = this.ResolveUrl("~/App_form/ns/td/ns_td_ungvien_nv_pd" + khac.Fs_runMode() + ".js?v=" + ht_dungchung.b_ver);
                ScriptManager.RegisterClientScriptInclude(this.Page, this.GetType(), Guid.NewGuid().ToString(), b_s);
                ScriptManager.RegisterStartupScript(this, this.GetType(), Guid.NewGuid().ToString(), "ns_td_ungvien_nv_pd_P_KD();", true);                
                P_NHAN_DROP();
            }
        }
    ...
    { 
        ...
        bang.P_THEM_HANG(ref b_dt, new object[] { "1", "Phê duyệt" });
        se.P_TG_LUU(this.Title, "TRANGTHAI_PD", b_dt.Copy());
    } 
}
