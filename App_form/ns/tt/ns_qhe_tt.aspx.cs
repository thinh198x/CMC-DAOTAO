...
public partial class f_ns_qhe_tt : fmau
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!IsPostBack)
            {
                ...
                string b_s = this.ResolveUrl("~/App_form/ns/tt/ns_qhe_tt" + khac.Fs_runMode() + ".js?v=" + ht_dungchung.b_ver);
                ScriptManager.RegisterClientScriptInclude(this.Page, this.GetType(), Guid.NewGuid().ToString(), b_s);
                ScriptManager.RegisterStartupScript(this, this.GetType(), Guid.NewGuid().ToString(), "ns_qhe_tt_P_KD('');", true);
    ...
    protected void XuatExcel_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                object[] a_object = ns_tt.Fdt_NS_QHE_TT_LKE(b_ma_cb, 1, 10000000);
                DataTable b_dt = (DataTable)a_object[1];
                bang.P_SO_CNG(ref b_dt, new string[] {  "ngayd"});
                bang.P_THAY_GTRI(ref b_dt,"phuthuoc","K","Không");
                bang.P_THAY_GTRI(ref b_dt, "phuthuoc", "C", "Có");
                b_dt.TableName = "DATA";
                Excel_dungchung.ExportTemplate("Templates/ExportMau/ns/hs/ns_qhe_tt.xlsx", b_dt, null, "Quan_he_nhan_than");
            }
        }
    ...
    {
        Excel_dungchung.ExportTemplate("Templates/importhsns/ns_qhe_tt.xls","ns_qhe_tt");
    }
}
