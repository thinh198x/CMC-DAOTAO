...
using Cthuvien;
public partial class f_ns_cb_ql : fmau
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!IsPostBack)
            {
                ...
                string b_s = this.ResolveUrl("~/App_form/ns/tt/ns_cb_ql" + khac.Fs_runMode() + ".js?v=" + ht_dungchung.b_ver);
    ...
    protected void XuatExcel_Click(object sender, EventArgs e)
    {
        try
        {
            object[] b_ds = ns_hs.Faobj_NS_CB_LKE_ALL(phong_an.Value,trangthai_an.Value,so_the_tk.Text,ten_tk.Text);
            var b_dt = (DataTable)b_ds[0];
            ...
            b_dt.TableName = "DATA";
            // ghi log
            hts_dungchung.PHT_LOG_NH(PHANHE.PD, NHOM_CHUCNANG.NGHIEP_VU, THAOTAC.EXPORT_EXCEL, TEN_FORM.NS_CB_QL, TEN_BANG.NS_CB_QL);
