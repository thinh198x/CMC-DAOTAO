...
function P_NH() {
    ...
    sns_hdns.Fs_NAM_BAI2_NH(form_Fs_nsd(), b_TrangKt, a_dt_ct, a_cot, P_NH_KQ, P_LOI_CSDL, P_LOI_TGIAN);
    return false;
}
...
function P_XOA() {
    ...
    if (b_ma == 0) { form_P_LOI('loi:Chọn bản ghi cần xóa:loi'); ns_hdns_ma_vtcdanh_P_MOI(); }
        ...
        sns_hdns.Fs_NAM_BAI2_XOA(form_Fs_nsd(), b_ma, a_tso, a_cot, a_cot_ct, P_XOA_KQ, P_LOI_CSDL, P_LOI_TGIAN);
    }
    return false;
}
...
function P_CHUYEN_HANG() {
    try {
        ...
        if (b_ma == "") {
            ...
            sns_hdns.Fs_NAM_BAI2_CT(form_Fs_nsd(), b_ma, P_CHUYEN_HANG_KQ, P_LOI_CSDL, P_LOI_TGIAN);
        }
    }
...
function P_LKE(b_nnn) {
    try {
        ...
        sns_hdns.Fs_NAM_BAI2_LKE(form_Fs_nsd(), a_tso, a_cot, b_nnn, ma_cd, ten_cd, P_LKE_KQ, P_LOI_CSDL, P_LOI_TGIAN);
        return false;
    }
    catch (err) { form_P_LOI(err); }
}
