...
function ns_hdns_ma_vtcdanh_P_MA_KTRA() {
    try {
        ...
        if (b_ma != "") {
            ...
            sns_hdns.Fs_NS_HDNS_NAM_BT3_MA(form_Fs_nsd(), b_ma, b_TrangKt, a_cot, ns_hdns_ma_vtcdanh_P_MA_KTRA_KQ, P_LOI_CSDL, P_LOI_TGIAN);
        }
    }
    catch (err) { form_P_LOI(err); }
}
...
function P_NH() {
    ...
    sns_hdns.Fs_NS_HDNS_NAM_BT3_NH(form_Fs_nsd(), b_TrangKt, a_dt_ct, a_cot, P_NH_KQ, P_LOI_CSDL, P_LOI_TGIAN);
    return false;
}
...
function P_XOA() {
    ...
    if (b_ma == 0) { form_P_LOI('loi:Chọn bản ghi cần xóa:loi'); P_MOI(); }
        ...
        sns_hdns.Fs_NS_HDNS_NAM_BT3_XOA(form_Fs_nsd(), b_ma, a_tso, a_cot, a_cot_ct, P_XOA_KQ, P_LOI_CSDL, P_LOI_TGIAN);
    }
    return false;
}
...
function P_CHUYEN_HANG() {
    try {
        ...
        if (b_ma == "") {
            ...
            sns_hdns.Fs_NS_HDNS_NAM_BT3_CT(form_Fs_nsd(), b_ma, P_CHUYEN_HANG_KQ, P_LOI_CSDL, P_LOI_TGIAN);
        }
    }
    catch (err) { form_P_LOI(err); }
}
...
function P_LKE() {
    try {
        var a_cot = GridX_Fas_tenCot(b_grlkeId), a_tso = slide_Faobj_TUDEN(b_slideId);
        sns_hdns.Fs_NS_HDNS_NAM_BT3_LKE(form_Fs_nsd(), a_tso, a_cot, P_LKE_KQ, P_LOI_CSDL, P_LOI_TGIAN);
    }
    catch (err) { form_P_LOI(err); }
}
...
function nhap_file() {//import từ file Excel
    ...
    form_P_MO(b_tenf, window.name, ["THAMSO", [window.name, b_so_the, "NAM_bai3", "Import dữ liệu - Hợp đồng"]], null);
    form_P_LOI('');
    return false;
