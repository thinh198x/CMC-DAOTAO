...
public partial class f_cc_khaibao_lamthem : fmau
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            string b_s = this.ResolveUrl("~/App_form/tl/cc/cc_khaibao_lamthem" + khac.Fs_runMode() + ".js?v=" + ht_dungchung.b_ver);
            ScriptManager.RegisterClientScriptInclude(this.Page, this.GetType(), Guid.NewGuid().ToString(), b_s);
            se.se_nsd b_se = new se.se_nsd();
            string b_nsd = b_se.nsd;
            string b_so_the = P_SOTHE(b_nsd);
            SO_THE.Text = b_nsd;
            ScriptManager.RegisterStartupScript(this, this.GetType(), Guid.NewGuid().ToString(), "cc_khaibao_lamthem_P_KD('');", true);            
    ...
    protected void nhap_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            b_dt.TableName = "DATA";
            Excel_dungchung.ExportTemplate("Templates/ExportMau/cc_khaibao_lamthem_export.xlsx", b_dt, null, "khaibao_lamthem");
