...
public partial class f_ns_cc_quetthe : fmau
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        { 
            if (!IsPostBack) 
            {
                ...
                string b_s = this.ResolveUrl("~/App_form/tl/cc/ns_cc_quetthe" + khac.Fs_runMode() + ".js?v=" + ht_dungchung.b_ver);
                ScriptManager.RegisterClientScriptInclude(this.Page, this.GetType(), Guid.NewGuid().ToString(), b_s);
                se.se_nsd b_se = new se.se_nsd();
                string b_nsd = b_se.nsd;
                string b_so_the = P_SOTHE(b_nsd);
                ScriptManager.RegisterStartupScript(this, this.GetType(), Guid.NewGuid().ToString(), "ns_cc_quetthe_P_KD('" + b_so_the + "');", true);
            } 
        }
