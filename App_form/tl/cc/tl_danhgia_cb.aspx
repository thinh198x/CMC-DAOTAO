<%@ Page Title="tl_danhgia_cb" Language="C#" MasterPageFile="~/trangnen.master" AutoEventWireup="true"
    CodeFile="tl_danhgia_cb.aspx.cs" Inherits="f_tl_danhgia_cb" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
        ...
        <tr class="css_tieudeN">
            <td colspan="2">
                <table cellpadding="0" cellspacing="0" width="100%">
                    <tr>
                        <td align="left">
                            <Cthuvien:luu ID="Luu1" runat="server" CssClass="css_tieudeM" Text="Đánh giá cán bộ" />
                        </td>
                        <td align="right">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <table border="0" cellspacing="0">
                                            <tr>
                                                <td>
                                                    <Cthuvien:DR_nhap ID="PHONG" runat="server" kieu="U" Width="200px" CssClass="css_drop"
                                                        onchange="tl_danhgia_cb_P_KTRA('PHONG')" DataTextField="ten" DataValueField="ma" />
                                                </td>
                                            </tr>
                                        </table>
                                    </td>
                            ...
                            <Cthuvien:GridX ID="GR_ct" runat="server" mauten="#9fc54e,#3a3a3a,#ffffff">
                                <%--<Bands>
                                    <igtbl:UltraGridBand AllowAdd="Yes">
                                        <Columns>
                                            <igtbl:UltraGridColumn BaseColumnName="so_the" AllowUpdate="No" Hidden="true">
                    ...
                    <tr>
                        <td align="center">
                            <table id="UPa_nhap" border="0" cellpadding="1" cellspacing="1">
                                <tr>
                                    <td>
                                        <Cthuvien:nhap ID="nhap" runat="server" Text="Nhập" CssClass="css_button" OnClick="return tl_danhgia_cb_P_NH();form_P_LOI();"
                                            Width="70px" />
                                    </td>
                                    <td>
                                        <Cthuvien:nhap ID="moi" runat="server" Text="Mới" CssClass="css_button" OnClick="return tl_danhgia_cb_P_MOI();form_P_LOI();"
                                            Width="70px" />
                                    </td>
                                    <td>
                                        <Cthuvien:nhap ID="xoa" runat="server" Text="Xóa" CssClass="css_button" OnClick="return tl_danhgia_cb_P_XOA();form_P_LOI();"
                                            Width="70px" />
                                    </td>
                                </tr>
                            </table>
            ...
            <td align="left" class="css_border">
                <div id="UPa_lke">
                    <Cthuvien:GridX ID="GR_lke" runat="server" ctr_sott="sott" mauten="#9fc54e,#3a3a3a,#ffffff"
                        DisplayLayout-ClientSideEvents-AfterRowActivateHandler="tl_danhgia_cb_GR_lke_ActiveRowChange">
                       <%-- <Bands>
                            <igtbl:UltraGridBand AllowAdd="Yes">
                                <Columns>
    ...
    <script language="javascript" type="text/javascript">
        var b_mt = "";
        function tl_danhgia_cb_P_KTRA(b_maTen) {
            try {
                var b_maId = "";
                ...
                if (b_maTen == "PHONG") {
                    tl_danhgia_cb_P_LKE();
                    tl_danhgia_cb_P_LKE_CB(b_ma.value);
                }
                    ...
                    if (b_hang > -1) {
                        ...
                        tl_danhgia_cb_P_CHUYEN_HANG(b_so_id);
                    }
                }
            }
            catch (err) { form_P_LOI(err); }
        }
        function tl_danhgia_cb_P_LKE_CB(b_phong) {
            try {
                var a_cot = Grid_Fas_TenCot('<%=GR_ct.ClientID%>');
                stl_cc.Fs_TL_DANHGIA_CB_LKE_CB(b_phong, a_cot, tl_danhgia_cb_P_LKE_CB_KQ, P_LOI_CSDL, P_LOI_TGIAN);
            }
            catch (err) { form_P_LOI(err); }
        }
        function tl_danhgia_cb_P_LKE_CB_KQ(b_kq) {
    ...
    <%-- Nhap --%>
    <script language="javascript" type="text/javascript">
        function tl_danhgia_cb_GR_lke_ActiveRowChange(gridId, rowId) {
            if (tl_danhgia_cb_choAct != 0) { clearTimeout(tl_danhgia_cb_choAct); tl_danhgia_cb_choAct = 0; }
            tl_danhgia_cb_choAct = setTimeout("tl_danhgia_cb_GR_lke_P_CHO()", 300);
            return true;
        }
        var tl_danhgia_cb_choAct = 0;
        function tl_danhgia_cb_GR_lke_P_CHO() {
            var b_so_id = Grid_Fobj_LayGtri_Act('<%=GR_lke.ClientID%>', "so_id");
            $get('<%=so_id.ClientID%>').value = b_so_id;
            tl_danhgia_cb_P_CHUYEN_HANG(b_so_id);
        }
        function tl_danhgia_cb_P_MOI() {
            ...
            tl_danhgia_cb_P_LKE_CB(b_phong);
            $get('<%=so_id.ClientID%>').value = "";
        }
        function tl_danhgia_cb_P_NH() {
            try {
                var b_vungId = '<%=UPa_ct.ClientID%>';
                ...
                stl_cc.Fs_TL_DANHGIA_CB_NH(b_so_id, a_dt, a_dt_ct, tl_danhgia_cb_NH_KQ, P_LOI_CSDL, P_LOI_TGIAN);
                return false;
            }
            catch (err) { throw (err); }
        }
        function tl_danhgia_cb_NH_KQ(b_kq) {
        ...
        function tl_danhgia_cb_P_CHUYEN_HANG(b_so_id) {
            if (b_so_id == null || b_so_id == "") { tl_danhgia_cb_P_MOI(); }
            else {
                var a_cot = Grid_Fas_TenCot('<%=GR_ct.ClientID%>');
                stl_cc.Fs_TL_DANHGIA_CB_CT(b_so_id, a_cot, tl_danhgia_cb_P_CHUYEN_HANG_KQ, P_LOI_CSDL, P_LOI_TGIAN);
            }
        }
        function tl_danhgia_cb_P_CHUYEN_HANG_KQ(b_kq) {
            if (Fb_LOI_KTRA(b_kq)) { form_P_LOI(b_kq); return; }
            var b_gridId = '<%=GR_ct.ClientID%>';
            if (b_kq == "") Grid_DatTrangCa(b_gridId);
        ...
        function tl_danhgia_cb_P_XOA() {
            try {
                var b_so_id = Grid_Fobj_LayGtri_Act('<%=GR_lke.ClientID%>', "so_id");
                if (b_so_id != "")
                    stl_cc.Fs_TL_DANHGIA_CB_XOA(b_so_id, tl_danhgia_cb_P_XOA_KQ, P_LOI_CSDL, P_LOI_TGIAN);
                return true;
            }
            catch (err) { throw (err); }
        }
        function tl_danhgia_cb_P_XOA_KQ(b_kq) {
            if (Fb_LOI_KTRA(b_kq)) { form_P_LOI(b_kq); return; }
            var b_gridId = '<%=GR_lke.ClientID%>';
        ...
        function tl_danhgia_cb_P_LKE() {
            try {
                var a_cot = Grid_Fas_TenCot('<%=GR_lke.ClientID%>'),
                    b_phong = $get('<%=PHONG.ClientID%>').value;
                stl_cc.Fs_TL_DANHGIA_CB_LKE(b_phong, a_cot, tl_danhgia_cb_P_LKE_KQ, P_LOI_CSDL, P_LOI_TGIAN);
            }
            catch (err) { form_P_LOI(err); }
        }
        function tl_danhgia_cb_P_LKE_KQ(b_kq) {
            var b_gridId = '<%=GR_lke.ClientID%>';
            if (b_kq == "") Grid_DatTrangCa(b_gridId);
            else Grid_P_DatBangCH(b_gridId, b_kq);
        }
