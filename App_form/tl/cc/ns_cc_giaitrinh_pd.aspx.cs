...
public partial class f_ns_cc_giaitrinh_pd : fmau
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        { 
            if (!IsPostBack) 
            {
                ...
                string b_s = this.ResolveUrl("~/App_form/tl/cc/ns_cc_giaitrinh_pd" + khac.Fs_runMode() + ".js?v=" + ht_dungchung.b_ver);
                ScriptManager.RegisterClientScriptInclude(this.Page, this.GetType(), Guid.NewGuid().ToString(), b_s);
                se.se_nsd b_se = new se.se_nsd();
                string b_nsd = b_se.nsd;
                string b_so_the = P_SOTHE(b_nsd);
                ScriptManager.RegisterStartupScript(this, this.GetType(), Guid.NewGuid().ToString(), "ns_cc_giaitrinh_pd_P_KD('" + b_so_the + "');", true);
                thang.Text = chuyen.NG_CTH(DateTime.Now);
                if (se.Fs_DUYET() != "IE") kthuoc.Value = "1050,700";
                tinhtrang.Focus();
            } 
        }
